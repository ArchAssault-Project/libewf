      ...
      FILE *stream )
{
	...
	fprintf( stream, "\t-c: specify the compression values as: level or method:level\n"
	                 ...
	                 "\t    compression level options: empty-block, fast (default) or \n"
	                 "\t    best\n" );
	...
	fprintf( stream, "\t-z: verbose of multithreading to stdout\n" );
         ...
         libcerror_error_t **error )
{
	...
	while( buffer_size > 0 )
	{
		...
		/* one chunk was read successfully
		 */
		buffer_size   -= read_size;
		buffer_offset += read_size;
	}
	/* whole buffer was read successfully
	 */
     ...
     libcerror_error_t **error )
{
	...
	if( init_threading_data_and_start_threads(
	     imaging_handle,
	     process_buffer_size,
	     &threading_data,
	     verbose_mt,
	    ...
	    || ( acquiry_count < (size64_t) imaging_handle->acquiry_size ) )
	{
		...
		if( read_count == 0 )
		{
			...
			{
				libcnotify_printf(
				 "%s: ewfacquirestream_read_chunk returned EOF.\n",
				 function);
			}
			break;
		}
		else
		{
			...
			{
				libcnotify_printf(
				 "%s: read_chunk returned: %" PRIzd ", raw_buffer_size: %" PRIzd ", read_size: %" PRIzd ".\n",
				 function, read_count, storage_media_buffer->raw_buffer_size, read_size);
			}
		}
		...
		if( process_count < 0 )
		{
			libcerror_error_set(
			 ...
			 LIBCERROR_ERROR_DOMAIN_IO,
			 LIBCERROR_IO_ERROR_READ_FAILED,
			 "%s: unable to put buffer into the reader_to_compressor_queue.",
			 function );
	...
	else
	{
		     ...
		     error ) == -1 )
		{
			libcerror_error_set(
			 ...
			 "%s: joining threads and cleanup of thread_data failed.",
			 function );
...
#endif
{
	                   ...
	                   _LIBCSTRING_SYSTEM_STRING( "A:b:B:c:C:d:D:e:E:f:hl:m:M:N:o:p:P:qsS:t:vVz2:" ) ) ) != (libcstring_system_integer_t) -1 )
	{
		switch( option )
		{
			...
			case (libcstring_system_integer_t) 'z':
				verbose_mt = 1;
	...
	ewfacquirestream_imaging_handle->compression_level = LIBEWF_COMPRESSION_FAST;
	ewfacquirestream_imaging_handle->compression_flags = 0;
	...
	if( option_compression_values != NULL )
	{
		...
		if( result == -1 )
		{
			...
			goto on_error;
		      ...
		      || ( ( ewfacquirestream_imaging_handle->compression_level == LIBEWF_COMPRESSION_NONE )
		       &&  ( ( ewfacquirestream_imaging_handle->compression_flags & LIBEWF_COMPRESS_FLAG_USE_EMPTY_BLOCK_COMPRESSION ) == 0 ) ) )
		{
			fprintf(
			 stderr,
			 "Unsupported compression values defaulting to method: deflate with level: fast.\n" );
